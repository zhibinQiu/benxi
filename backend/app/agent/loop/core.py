...
def build_turn_tools_context(
    ...
    *,
    ...
    """从 loop_state 提取本轮已执行工具的短摘要（纯 app.agent，无平台依赖）。"""
    state = loop_state or {}
    lines: list[str] = []
    records = list(state.get("executed_tool_calls") or [])
    for rec in records:
        if not isinstance(rec, dict):
            continue
        name = str(rec.get("name") or rec.get("tool") or "").strip()
        summary = str(rec.get("summary") or "").strip()
        if not summary:
            continue
        text = f"{name}: {summary}" if name else summary
        lines.append(text[:line_limit])
        if len(lines) >= max_lines:
            break
    if not lines:
        for raw in list(state.get("tool_outcome_lines") or [])[-max_lines:]:
