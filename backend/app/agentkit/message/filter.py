...
from app.agentkit.message.parse import content_has_tool_markup
...
_INTERNAL_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    # 路由标记（LLM 可能在回复中写 ROUTE / __ROUTE__ 表示"需要路由"）
    re.compile(r"\b__ROUTE__\b|\bROUTE\b"),
)
...
def sanitize_agent_reply(
    ...
    *,
    ...
    """
    raw = text or ""
    from app.agentkit.message.parse import extract_embedded_tool_calls
