...
from app.schemas.document import DepartmentPickerOut
...
@router.get("/tree", response_model=ApiResponse[list[DepartmentPickerOut]])
def department_tree(
    db: Annotated[Session, Depends(get_db)],
    _: Annotated[User, Depends(get_current_user)],
) -> ApiResponse[list[DepartmentPickerOut]]:
    """发布到文库时的部门树选择器：返回全部部门层级结构。"""
    ...
    return ApiResponse(
        data=[
            DepartmentPickerOut(id=d.id, name=d.name, parent_id=d.parent_id)
            for d in rows
        ]
    )
