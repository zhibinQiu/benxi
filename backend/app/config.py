...
class Settings(BaseSettings):
    ...
    knowflow_theme_app_name: str = "本析"
    ...
    # 多智能体：Supervisor 路由 + 子智能体专精 tool loop
    ...
    # Skill 匹配：归一化相似度下限（低于则走能力兜底，不强行路由专精 Agent）
    agent_skill_match_threshold: float = 0.3
    # Human-in-the-Loop: 需要用户确认的工具列表（逗号分隔；空则使用内置默认集）
    hitl_confirm_tools: str = ""
    # 无匹配兜底：loose=能做多少做多少；strict=有缺失则整单终止说明
    agent_capability_fallback_mode: str = "loose"
    # 调度层：子智能体 request_orchestrator_assist 后的协助与续办
    agent_orchestrator_max_assist_rounds: int = 2
    # 调度层全局 Loop：验收未通过时写反思并重规划（始终开启，最低 2 轮，见 supervisor_max_global_rounds）
    ...
    # MCP Skill 互联：外部 MCP Server 接入与平台 Skill 对外暴露
    ...
    # 资讯管理 · 在线搜索（SearXNG JSON API 基址，如 http://host:8080）
    searxng_url: str = ""
