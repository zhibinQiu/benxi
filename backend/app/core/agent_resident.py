    ...
    *,
    ...
    if agent == "orchestrator":
        ...
        return (
            ...
            "⊙ **只委托、不直执**：你只能调用 invoke_context_subagent / "
            "request_orchestrator_assist / ask_user_choice / find_skills / describe_tool / search_tools。\n"
            "⊙ **领域明确** → 必须通过以下方式委托，禁止只用文字描述：\n"
            "   · invoke_context_subagent(kind=search) — 联网检索/知识库/图谱/本体\n"
            "   · invoke_context_subagent(kind=use) — 执行已有 Skill\n"
            "   · invoke_context_subagent(kind=execute, steps=[...]) — 浏览器/通知等明确步骤\n"
            "   · request_orchestrator_assist(agent_id=...) — 路由到专精 Agent\n"
            "⊙ **禁止**直接调用 web_search / knowledge_retrieve / browser_* / 通知等原子工具。\n"
            "⊙ **禁止只说不做**：确定需要检索或执行时，同一轮必须发起 invoke_context_subagent。\n"
            "══════════════════════════════════════════\n"
