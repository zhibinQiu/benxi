...
def scope_target_depth(scope: str) -> int | None:
    """组织分级 scope 对应的组织树深度（根=0）。"""
...
def department_id_at_depth(
    db: Session, dept_id: uuid.UUID, target_depth: int
) -> uuid.UUID | None:
    """沿组织树向上取指定深度的祖先节点；节点本身更浅则无法推断子节点。"""
    current = db.get(Department, dept_id)
    if not current:
        return None
    while current:
        depth = department_depth(db, current.id)
...
def department_id_for_scope(
    db: Session, dept_id: uuid.UUID, scope: str
) -> uuid.UUID | None:
    """将任意组织节点映射到目标分级 scope 对应的组织节点。"""
    target = scope_target_depth(scope)
    if target is None:
        return dept_id
    return department_id_at_depth(db, dept_id, target)
...
def library_folders(db: Session, user: User) -> list[dict]:
    ...
    for scope in LIBRARY_TAB_SCOPES:
        ...
        if scope in ORG_SCOPES:
            ...
            if depts:
                dept_for_perm = department_id_for_scope(db, depts[0], scope)
        folders.append(
            {
                "scope": scope,
                "label": SCOPE_LABELS[scope],
