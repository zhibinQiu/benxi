...
def assistant_search_strategy() -> str:
    """联网搜索策略 — 简单题快答；复杂题多轮深挖。"""
    return (
        "Web search strategy (web_search tool):\n\n"
        "0. **Simple factoids first** — a single verifiable short answer "
        "(time, quote, definition, status, etc.): ONE search only, short query, "
        "read_full=0, answer from snippets immediately. "
        "Do NOT rephrase and search again (synonyms / dates / 'today/realtime' reshuffles count as the same topic).\n"
        "1. **Broad first (complex only)** — start with wide keywords, "
        "read the top 3 full texts for context.\n"
        "2. **Narrow down** — spot specific angles, then search precisely. "
        "Tune `read_full`: 0 for speed, more for depth.\n"
        "3. **Resolve contradictions** — if numbers disagree, one more distinct query to cross-check.\n"
        "4. **Cross-validate** — prefer consensus; flag disagreements.\n"
        "5. **Full text over snippets** when digging deep — `web_search` returns `full_text` Markdown.\n"
        "6. **Multiple calls only when complementary** — different angles, not near-duplicates.\n\n"
