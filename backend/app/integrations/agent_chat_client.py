...
async def iter_agent_chat_stream(
    *,
    ...
    key = _clean(api_key)
    if not base or not key:
        yield _dumps({"error": f"{feature_label}未配置对话服务"})
        return
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(300.0)) as client:
            async with client.stream("POST", url, headers=headers, json=payload) as r:
                if r.status_code >= 400:
                    ...
                    yield _dumps({"error": f"{feature_label}服务请求失败: {body}"})
                    return
                ...
                async for line in r.aiter_lines():
                    ...
                    if ev == "message_replace":
                        ...
                        if replacement:
                            ...
                            yield _dumps({"replace": replacement})

                    wf = _workflow_sse_payload(event)
                    if wf:
                        yield _dumps({"workflow": wf})

                    if ev == "message_end":
                        cites = parse_dify_citations(event)
                        if cites:
                            accumulated_citations = cites
                            yield _dumps({"citations": cites})

        if not accumulated_answer.strip():
            yield _dumps({"error": f"{feature_label}服务返回为空"})
            return
        ...
        if accumulated_answer.strip():
            ...
            return
        yield _dumps({"error": f"无法连接{feature_label}服务: {e}"})
