...
def fetch_web_article(url: str, *, timeout: float = 25.0) -> ParsedFeedEntry:
    ...
    并行尝试所有 URL 候选（http↔https、www 变体），从中挑选最佳结果。
    """
    site_kind = detect_site_kind(url)
    candidates = _candidate_urls(url)
    if not candidates:
        raise WebArticleFetchError("无法解析链接，请确认 URL 格式正确")
    ...
    def _try_single_candidate(
        ...
        """单个 URL 候选：用多组请求头尝试，返回最佳结果。"""
        with httpx.Client(timeout=timeout, follow_redirects=True) as client:
            best_attempt: _FetchAttempt | None = None
            best_score = -1
            for idx, headers in enumerate(
                _request_header_sets(candidate_url)
            ):
                attempt = _attempt_fetch(client, candidate_url, headers)
                if attempt is None:
                    if idx == 0:
                        return None
                    continue
    ...
    with ThreadPoolExecutor(
        ...
        future_map = {
            executor.submit(_try_single_candidate, c): c
            for c in candidates
        }
