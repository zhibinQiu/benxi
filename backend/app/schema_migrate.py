...
_CURRENT_PLATFORM_APP_TITLE = "本析"
...
def ensure_agent_skill_schema(engine: Engine) -> None:
    statements = [
        ...
        "ALTER TABLE agent_skills ADD COLUMN IF NOT EXISTS needs_review BOOLEAN NOT NULL DEFAULT FALSE",
        "ALTER TABLE agent_skills ADD COLUMN IF NOT EXISTS mount_agent VARCHAR(32)",
        "CREATE INDEX IF NOT EXISTS ix_agent_skills_needs_review ON agent_skills (needs_review)",
        """
...
def ensure_mcp_external_skills_schema(engine: Engine) -> None:
    """外部 MCP Skill 登记（HTTP/SSE 接入第三方 MCP Server）。"""
    statements = [
        """
        CREATE TABLE IF NOT EXISTS mcp_external_skills (
            id UUID PRIMARY KEY,
            name VARCHAR(64) NOT NULL UNIQUE,
            title VARCHAR(256) NOT NULL DEFAULT '',
            description TEXT NOT NULL DEFAULT '',
            ...
            dont_use_when TEXT NOT NULL DEFAULT '',
        ...
        """,
        """
        CREATE INDEX IF NOT EXISTS ix_mcp_external_skills_name
        ON mcp_external_skills (name)
        """,
    ]
    with engine.begin() as conn:
        for sql in statements:
