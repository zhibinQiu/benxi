...
- 本体定义：左侧菜单或功能列表 → 本体定义；管理类型/公理（模式层）与实体实例、图谱探索、LLM 抽取（实例层）。
- 本析智能：功能列表 → 工具 → 本析智能；或左侧菜单直达；小析联合文档检索与本体实例图谱回答。
- 智能问数 / 双碳问答：功能列表 → 对应卡片，小析多轮对话与引用溯源。
- 文档对比：功能列表 → 文档对比，从文档库选择 PDF/Word。
- 语音转写 / 语音合成 / 文件内容提取：功能列表 → 对应卡片。
- 资讯收录：知识中心 → 资讯管理（粘贴文章链接 / 微信公众号）。
...
- **资讯管理**：个人文档库系统文件夹，名称「资讯管理」；通过资讯管理粘贴链接或公众号导入的文档自动归入。小析可用文档库工具列出其中文件，与浏览器书签无关。
