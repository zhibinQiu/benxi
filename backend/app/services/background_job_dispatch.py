...
def dispatch_document_index_job(job_id: uuid.UUID, *, countdown: int = 0) -> None:
    """文档索引 / 重新索引 Job 调度（含解析等待阶段的延迟续跑）。

    实现思路：读 Job payload → PageIndex 或资讯导入链则跳过 Celery，走 ``submit_background``；
    否则优先 Celery Worker，失败再回落线程池。完整执行链见
    ``knowledge_sync_job_service.run_document_knowledge_index_job``。
    """
...
def dispatch_scheduled_notification(
    ...
    短倒计时（<60s）强制进程内执行，避免 Celery Worker 未启动时任务入队但无人消费。
    """
