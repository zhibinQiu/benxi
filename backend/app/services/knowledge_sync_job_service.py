...
执行模型见 ``document_index_coordinator``：单 Job 单执行者、单调度链、入队幂等。
"""
...
from app.services.document_index_coordinator import (
    clear_parse_phase_fields,
    dispatch as dispatch_index_job,
    end_execution,
    enter_awaiting_parse_phase,
    ensure_dispatched,
    is_awaiting_parse_phase,
    parse_watch_started_at,
...
def _collect_document_index_jobs(
    ...
    *,
    ...
    for job in candidates:
        ...
        if (
            job_version == version_str
            or job_rid in ragflow_ids
            or (not job_version and (payload.get("document_id") or "").strip() == doc_str)
        ):
            matched.append(job)
    return matched
...
def stop_document_index_work(
    ...
    *,
    ...
    for job in _collect_document_index_jobs(db, document_id, version_id=version_id):
        payload = job.payload or {}
        had_parse_phase = is_awaiting_parse_phase(payload)
...
def _try_claim_index_job_terminal(
    ...
    *,
    ...
    if clear_parse_watch:
        payload = clear_parse_phase_fields(payload)
...
def _complete_subscription_pipeline_after_deepdoc(
    ...
    *,
    ...
    payload = clear_parse_phase_fields(dict(job.payload or {}))
    job.payload = payload
...
def _run_awaiting_parse_phase(job_id: uuid.UUID) -> None:
    """index_phase=awaiting_parse：仅轮询 KnowFlow 解析，完成后终态化 Job。"""
    from app.config import get_settings
    from app.database import SessionLocal
    ...
    try:
        ...
        started_at = parse_watch_started_at(payload) or time.time()
    ...
    try:
        ...
        if parse_exc is not None:
            _fail_parse_job(db, job, user, doc, str(parse_exc), mode=mode)
            db.commit()
            return

        if completed:
            db.refresh(job)
            if not is_awaiting_parse_phase(job.payload if isinstance(job.payload, dict) else {}):
                return
...
def run_document_knowledge_index_job(job_id: uuid.UUID) -> None:
    ...
    try:
        ...
        if job.status not in (JobStatus.pending.value, JobStatus.running.value):
            return
        if not try_begin_execution(db, job):
            logger.info("文档索引任务已有执行者，跳过 job=%s", job_id)
            return
        ...
        if is_awaiting_parse_phase(payload):
            db.commit()
            _run_awaiting_parse_phase(job_id)
            return
        ...
        if execution_started:
            try:
                job = db.get(Job, job_id)
                if job:
                    end_execution(db, job)
                    db.commit()
            except Exception:
                logger.debug("释放文档索引租约失败 job=%s", job_id, exc_info=True)
        db.close()


def _start_job_thread(job_id: uuid.UUID) -> None:
    dispatch_index_job(job_id)
...
def create_document_knowledge_index_job(
    ...
    *,
    ...
    if existing and not force:
        logger.info(
            "复用进行中的文档索引任务 doc=%s version=%s job=%s",
            document_id,
            version_id,
            existing.id,
        )
    ...
    if existing and force:
        logger.info(
            "强制重建：取消旧文档索引任务 doc=%s version=%s job=%s",
            document_id,
            version_id,
            existing.id,
        )
