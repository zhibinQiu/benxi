...
WEB_FAVORITES_FOLDER_NAME = "资讯管理"

SYSTEM_FOLDER_HINTS = {
    FOLDER_KIND_UNCATEGORIZED: "尚未归入自定义文件夹的文档；不可删除或重命名。",
    FOLDER_KIND_WEB_FAVORITES: "通过资讯管理功能导入的公众号、网页文档会自动归入此文件夹。",
}
...
def _normalize_folder_scope(
    ...
    *,
    ...
    if scope in ORG_SCOPES:
        ...
        if dept_id is None:
            user_dept = primary_dept_id(db, user.id) or user_depts[0]
            dept_id = department_id_for_scope(db, user_dept, scope)
            if dept_id is None:
                raise bad_request("当前组织归属与所选分级不匹配")
