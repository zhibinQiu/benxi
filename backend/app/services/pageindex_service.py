...
def parsed_document_from_pageindex(
    db: Session, doc: Document
) -> "ParsedDocument | None":
    """从已建 PageIndex 树读取正文，供问答本地检索/全文读取（免重复解析）。"""
    from app.integrations.pageindex_bridge import (
    ...
    if not link or not link.index_completed_at:
        return None
    pi_doc_id = (link.pageindex_doc_id or "").strip()
    if not pi_doc_id:
        return None
    stored = load_pageindex_doc(pageindex_workspace_dir(), pi_doc_id)
    if not stored:
        return None
    full_text = flatten_pageindex_structure_text(stored.get("structure") or [])
    if not full_text.strip():
        return None
    file_name = (link.file_name or doc.title or "document").strip() or "document"
...
def load_qa_parsed_documents(db: Session, docs: list[Document]) -> list["ParsedDocument"]:
    """问答回退解析：优先 PageIndex 缓存正文，缺失再现场 DeepDOC/OCR。"""
    from app.integrations.text_extract import ParsedDocument
    ...
    for doc in docs:
        parsed = parsed_document_from_pageindex(db, doc)
        if parsed:
            out.append(parsed)
        else:
    ...
    if remaining:
        ...
        out.extend(load_parsed_documents(db, remaining))
    return out


def get_pageindex_document_content(
    db: Session,
    user: User,
    document_id: uuid.UUID,
) -> dict[str, Any] | None:
    """读取 PageIndex 已索引文档正文（问答/Agent 全文读取 fast path）。"""
