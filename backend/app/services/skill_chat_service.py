...
async def resolve_combined_research_async(
    ...
    *,
    ...
    channels = {"kb": False, "kg": False, "web": False}
    kg_context: KgQaContext | None = None
    merged_context = ""
    citations: list[dict] = []

    async def _run_kg() -> None:
    ...
    async def _run_kb() -> None:
        ...
        if doc_context.strip():
            parts = [p.strip() for p in (merged_context, doc_context) if p.strip()]
            merged_context = "\n\n".join(parts)
            citations = list(citations) + list(doc_citations)
    ...
    if plan.explicit:
        ...
        if plan.run_kb:
            await _run_kb()
        return merged_context, citations, kg_context, channels

    if plan.run_kg:
        await _run_kg()
        if kg_has_material(kg_context) or context_text_sufficient(merged_context):
            return merged_context, citations, kg_context, channels

    if plan.run_web:
        await _run_web()
        if citations_have_sources(citations, _WEB_CITATION_SOURCES) or context_text_sufficient(
            merged_context
        ):
            return merged_context, citations, kg_context, channels
