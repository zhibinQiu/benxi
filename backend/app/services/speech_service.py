...
from app.integrations.video_url_fetcher import fetch_media_from_url, ytdlp_available
...
async def get_meta() -> SpeechMetaOut:
    ...
    if remote:
        ...
        return SpeechMetaOut(
            ...
            service_hint=hint,
            video_url_available=ytdlp_available(),
        )
...
async def transcribe_from_url(
    *,
    ...
    content, filename = await fetch_media_from_url(url, max_bytes=max_bytes)
...
def import_meeting_summary_to_library(
    ...
    *,
    ...
    """将会议智能总结导出为 Word 并写入个人级文档库（默认未分类）。"""
    blocks = list(summary_blocks or [])
    markdown = blocks_to_markdown(blocks) if blocks else (summary or "").strip()
    if not markdown:
        raise bad_request("总结内容为空")

    from app.services.report_generation_service import import_report_to_library

    return import_report_to_library(
        db,
        user,
        title=title,
        markdown=markdown,
