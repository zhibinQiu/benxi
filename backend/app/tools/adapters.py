...
async def _run_kg_query(ctx: ToolRuntimeContext, params: dict[str, Any]) -> tuple[bool, str, dict | None]:
    """知识图谱查询 — 经 KgQueryService 执行本体规划的 Neo4j 路径。"""
    ...
    try:
        ...
        plan = await hub.plan_query(
            question,
            owner_id=str(ctx.user.id),
            max_depth=3,
            include_inferred=True,
        )
        payload = await kg.reason(
            question,
            str(ctx.user.id),
            max_depth=3,
            include_inferred=True,
...
async def _run_ontology_query(ctx: ToolRuntimeContext, params: dict[str, Any]) -> tuple[bool, str, dict | None]:
    """语义中枢：概念理解 + 字段映射 + 查询计划（非整库 dump，不查事实库）。"""
    question = str(params.get("question") or params.get("query") or "").strip()
    from app.core.permissions import user_has_semantic_layer_permission
    ...
    try:
        from app.services.semantic_runtime import get_ontology_hub_service

        hub = await get_ontology_hub_service()
        plan = await hub.plan_query(question)
        ontology_text = await hub.explain_for_tool(question)
        ...
        return _tool_result(
            ...
            "本体语义决策完成",
            {
                ...
                "field_bindings": [
                    {
                        "concept": b.concept,
                        "property": b.property_key,
                        "source": b.source,
                        "table": b.table,
                        "column": b.column,
                        "neo4j_prop": b.neo4j_prop,
                    }
                    for b in plan.field_bindings[:24]
                ],
                "preferred_tools": plan.preferred_tools,
                "sql_plan": [
                    {"description": s.description, "sql": s.sql, "table": s.table}
                    for s in plan.sql_steps
                ],
                "neo4j_plan": [
                    {"description": s.description} for s in plan.neo4j_steps[:12]
                ],
            },
        )
    except Exception as exc:
...
def _append_carbon_evidence(
    ...
    *,
    ...
    from app.tools.agent_bridge import _citation_start
...
async def _run_carbon_policy(
    ...
    """双碳政策法规：发改委智能云搜索 + 详情摘要列表。"""
    from app.services import carbon_service as svc

    keyword = svc.normalize_policy_keyword(str(params.get("keyword") or "").strip())
    url = str(params.get("url") or "").strip()
    data = await svc.fetch_carbon_policy(keyword=keyword, url=url)
    ok = bool(data.get("ok"))
    summary = str(data.get("summary_md") or "")[:20000]
    sources = list(data.get("sources") or [])
    n = len(sources)
    title = f"已获取 {n} 条政策" if ok else "政策数据源暂时无法访问"
    if ok:
        _append_carbon_evidence(
