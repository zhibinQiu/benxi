...
def test_stream_emits_citations_before_done():
    ...
    payloads = asyncio.run(_collect_stream(lines))
    cite_events = [p for p in payloads if p.get("citations") and not p.get("done")]
