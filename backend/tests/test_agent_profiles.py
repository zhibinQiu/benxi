...
def test_list_external_agents_route(client, admin_token):
    ...
    # 兼容误请求 /external（否则会被 /{skill_id} 捕获并报 UUID 校验错误）
    r_compat = client.get("/api/v1/admin/agent-skills/external", headers=headers)
    assert r_compat.status_code == 200, r_compat.text
    assert r_compat.json()["data"] == r.json()["data"]
