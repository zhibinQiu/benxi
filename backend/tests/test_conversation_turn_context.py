...
def test_is_likely_follow_up_greeting_after_business_is_not_follow_up():
    history = [
        AiChatMessage(
            role="user",
    ...
    assert is_likely_follow_up("你好", history) is False


def test_standalone_question_not_treated_as_follow_up():
    history = _history()
    q = "把大象放进冰箱需要几步"
    assert is_standalone_question(q) is True
    assert is_likely_follow_up(q, history) is False


def test_build_turn_planning_context_omits_history_for_new_topic():
    ctx = build_turn_planning_context("把大象放进冰箱需要几步", _history())
    assert "当前输入" in ctx
    assert "碳价格" not in ctx
    assert "近期对话" not in ctx


def test_effective_question_for_retrieval_keeps_new_topic_isolated():
    text = effective_question_for_retrieval("把大象放进冰箱需要几步", _history())
    assert text == "把大象放进冰箱需要几步"
    assert "碳价格" not in text
...
def test_needs_knowledge_retrieval_for_follow_up_after_business():
    history = [
        AiChatMessage(role="user", content="帮我检索知识库里的碳配额流程"),
