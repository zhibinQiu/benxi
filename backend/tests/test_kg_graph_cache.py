...
def test_kg_entities_served_from_cache(client: TestClient, admin_token: str):
    headers = {"Authorization": f"Bearer {admin_token}"}
    meta = client.get("/api/v1/kg/meta", headers=headers)
    assert meta.status_code == 200
    type_id = meta.json()["data"]["entity_types"][0]["id"]
    ...
    create = client.post(
        "/api/v1/kg/entities",
        headers=headers,
        json={"type_id": type_id, "name": "实体列表缓存测试", "description": ""},
    )
    assert create.status_code == 200
    entity_id = create.json()["data"]["id"]
    ...
    def counting_entities(*args, **kwargs):
        calls["count"] += 1
        return original(*args, **kwargs)

    from app.services import kg_service

    original = kg_service._entities_out
    with patch.object(kg_service, "_entities_out", side_effect=counting_entities):
        first = client.get("/api/v1/kg/entities", headers=headers)
        second = client.get("/api/v1/kg/entities", headers=headers)
