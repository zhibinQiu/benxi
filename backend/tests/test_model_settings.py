...
def test_service_urls_from_env_defaults(monkeypatch):
    settings = Settings(
        ...
        searxng_url="http://10.0.0.1:40000",
    ...
    assert out.searxng_url == "http://10.0.0.1:40000"
    ...
    assert get_searxng_url(None) == "http://10.0.0.1:40000"
...
def test_platform_api_base_url_from_env(monkeypatch):
    settings = Settings(
        ...
        platform_api_base_url="http://10.0.0.1:40005/ai",
    )
    monkeypatch.setattr(
        "app.services.model_settings_service.get_settings",
        lambda: settings,
    ...
    out = get_model_settings(None)
    assert out.platform_api_base_url == "http://10.0.0.1:40005/ai"
    assert get_platform_api_base_url(None) == "http://10.0.0.1:40005/ai"
