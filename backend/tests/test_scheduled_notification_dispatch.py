...
def test_dispatch_short_countdown_bypasses_celery(monkeypatch):
    """短倒计时（<60s）直接走进程内 Timer，不尝试 Celery。"""
    timer_calls: list[tuple[float, object]] = []
    inprocess_calls: list[uuid.UUID] = []
    celery_called: list[bool] = []
    ...
    monkeypatch.setattr(dispatch_mod, "_try_celery", lambda *a, **k: celery_called.append(True) or True)

    nid = uuid.uuid4()
    dispatch_mod.dispatch_scheduled_notification(nid, countdown=8)

    assert celery_called == [], "短倒计时不应调用 _try_celery"
    assert len(timer_calls) == 1
    assert timer_calls[0][0] == 8
    timer_calls[0][1]()
    assert inprocess_calls == [nid]
