...
@patch("app.services.speech_service.transcribe_from_url", new_callable=AsyncMock)
def test_speech_transcribe_url_success(mock_transcribe_url, client, admin_token):
    from app.schemas.speech import SpeechSegmentOut, SpeechTranscribeOut

    mock_transcribe_url.return_value = SpeechTranscribeOut(
        text="视频转写内容",
        language="zh",
        ...
        segments=[
            SpeechSegmentOut(speaker="说话人 1", start=0.0, end=30.0, text="视频转写内容")
        ],
    )
    r = client.post(
    ...
    assert r.status_code == 200, r.text
    assert r.json()["data"]["text"] == "视频转写内容"
    mock_transcribe_url.assert_awaited_once()
...
def test_speech_import_library(client, admin_token):
    doc_id = uuid.uuid4()
    with patch(
        "app.services.report_generation_service.import_report_to_library",
        return_value={
            "document_id": doc_id,
            "title": "周会纪要",
            "knowflow_synced": False,
            "message": "已添加到个人级文档库（未分类）",
        },
    ):
        r = client.post(
            "/api/v1/speech/import-library",
            headers={"Authorization": f"Bearer {admin_token}"},
            json={
                "title": "周会纪要",
                "summary": "## 要点\n讨论进度",
