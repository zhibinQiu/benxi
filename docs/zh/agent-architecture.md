...
│  ② 专精 Agent（Specialist）                              │
│     完整 tool loop，面向用户                             │
│     platform / report / skill-dev / carbon /             │
│     power-economy / stock                                │
...
│     search / use / execute                               │
│     → 父 Agent 在流程中按需调用                          │
│     → 有自己的 LLM 循环，自主决策（execute 按 steps）    │
│     → 结果汇总回父 Agent 的上下文                        │
└──────────────────────────────────────────────────────────┘
```
...
| 事件类型 | 路由目标 | 示例 |
|---------|---------|------|
| 信息获取（查数据、做调研） | 编排器自留，tool loop 内解决 | "查一下碳价格" → 编排器调 `web_search` 或 `carbon_price` |
| 深度信息获取（需多轮分析） | 编排器调子 Agent | "分析碳市场趋势" → `invoke_context_subagent(kind=search)` |
| 领域操作（写报告、创建 Skill、文档操作） | 专精 Agent | "帮我创建监控碳价的 Skill" → `skill-dev` |
| 专精工作流中的一环（浏览器步骤、批处理） | 子 Agent `execute` | 父层编排导航/点击步骤 → `kind=execute` |
...
```
① 事件是领域操作（写报告、创建 Skill、操作文档、定时任务、双碳/股市专业分析）
   → 专精 Agent

② 事件是信息获取（查数据、做调研、找资料）
   → 编排器自留，tool loop 内决定：
       简单查 → 直接调原子工具（web_search / carbon_price / stock_quote）
       深度研究 → 委托子 Agent（invoke_context_subagent kind=search）

③ 事件是专精工作流中的一环（需要按步骤执行的浏览器/批处理操作）
   → 子 Agent kind=execute
```
