...
企业 AI 知识库平台使用 **单一 Docker Compose 栈**（项目名 `benxi`），所有服务在同一 Docker 网络 `benxi` 内通信。
...
|--------------|----------------|------|----------|------------|------|
| `frontend` | `benxi-frontend-1` | benxi-frontend | 80 | **40005** | 唯一 Web 入口；反代 SPA、API、KnowFlow iframe |
| `api` | `benxi-api-1` | benxi-api | 8000 | 开发时 **18000** | FastAPI：鉴权、文档、插件、embed-proxy |
| `worker` | `benxi-worker-1` | benxi-api | — | 不映射 | Celery 异步任务 |
| `postgres` | `benxi-postgres-1` | postgres:16-alpine | 5432 | 不映射 | 平台业务库 |
| `redis` | `benxi-redis-1` | redis:7-alpine | 6379 | 不映射 | Celery 队列、缓存 |
| `minio` | `benxi-minio-1` | minio/minio | 9000/9001 | 不映射 | S3 对象存储 |
| `pdf2zh-api` | `benxi-pdf2zh-api-1` | benxi-pdf2zh | 7861 | 不映射 | PDF 翻译 REST |
| `speech-api` | `benxi-speech-api-1` | benxi-speech | 8765 | 不映射 | 语音转写（profile `speech`） |
...
```bash
# 进入交互式 psql
docker compose -p benxi exec -it postgres \
...
```bash
docker compose -p benxi exec -T postgres \
...
```bash
docker compose -p benxi exec -it knowflow-mysql \
...
```bash
# 无密码时
docker compose -p benxi exec -it redis redis-cli

# 有 REDIS_PASSWORD 时
docker compose -p benxi exec -it redis redis-cli -a "$REDIS_PASSWORD"
...
```bash
docker compose -p benxi exec -it minio sh -c '
...
```bash
docker compose -p benxi run --rm -p 127.0.0.1:9001:9001 --no-deps minio \
...
```bash
docker compose -p benxi exec -it ragflow-infinity \
