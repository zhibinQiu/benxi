...
仅暴露 **40005**。API 文档内网：`docker compose -p benxi exec api curl -s localhost:8000/health`。
...
Python 依赖打进 **`benxi-api-runtime`** 镜像；`platform/app` 挂载进容器，改代码无需重建镜像。
...
| 镜像 | 用途 |
|------|------|
| `benxi-api-runtime` | 仅 Python 依赖，运行时挂载 `platform/app` |
| `benxi-api`（production） | 依赖 + 代码 baked-in，用于 `stack save` 离线包 |
