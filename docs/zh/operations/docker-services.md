...
统一栈项目名 **`benxi`**，网络 **`benxi`**（bridge）。除 `frontend` 外，**默认不映射主机端口**。
...
|--------|--------------|------|----------|------|
| `benxi-postgres-1` | postgres | postgres:16-alpine | 5432 | 平台业务库：用户、RBAC、文档元数据、任务、审计 |
| `benxi-redis-1` | redis | redis:7-alpine | 6379 | Celery broker、缓存、KnowFlow 可选 Redis |
| `benxi-minio-1` | minio | minio/minio | 9000/9001 | 文档对象存储；KnowFlow 共用 |
| `benxi-pdf2zh-api-1` | pdf2zh-api | benxi-pdf2zh:${BENXI_VERSION} | 7861 | PDF 翻译 REST；BabelDOC 模型预热 |
| `benxi-api-1` | api | benxi-api:${BENXI_VERSION} | 8000 | FastAPI：鉴权、文档、插件 API、embed-proxy |
| `benxi-worker-1` | worker | 同 api | — | Celery：翻译、对比、同步 KnowFlow 等长任务 |
| `benxi-frontend-1` | frontend | benxi-frontend 或 dev 时 node:22 | 80 | **唯一对外** `${FRONTEND_PORT:-40005}`；Nginx 或 Vite |
...
| 容器 | 镜像 | 端口 | 职责 |
|------|------|------|------|
| `benxi-speech-api-1` | benxi-speech | 8765 | FunASR 转写；模型卷 `${DATA_ROOT}/speech-models` |
...
| 容器 | 镜像 | 端口 | 职责 |
|------|------|------|------|
| `benxi-docs-1` | benxi-docs | **40100**（默认） | MkDocs Material 系统文档；挂载 `docs/zh` 热更新 |
...
```bash
docker compose -p benxi ps
docker compose -p benxi logs -f api
docker compose -p benxi logs -f ragflow-server   # 需 knowflow profile
