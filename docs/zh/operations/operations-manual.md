...
```bash
docker compose -p benxi logs -f api worker frontend
docker compose -p benxi logs -f ragflow-server --tail 100
docker compose -p benxi logs -f speech-api
```
...
```bash
# 平台业务库
docker compose -p benxi exec -it postgres psql -U platform -d platform

# KnowFlow 元数据（密码见 .env MYSQL_PASSWORD）
docker compose -p benxi exec -it knowflow-mysql mysql -uroot -p'infini_rag_flow' rag_flow
```
...
docker compose -p benxi logs api --tail 50
```
...
```bash
docker compose -p benxi logs pdf2zh-api
