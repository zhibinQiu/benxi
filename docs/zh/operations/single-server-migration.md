...
```bash
# 1. 获取代码与配置
git clone <仓库> /opt/benxi    # 或 rsync 现有目录
cd /opt/benxi
...
docker compose -p benxi exec api curl -s localhost:8000/health
```
...
| **Worker** | 挂载 `./platform/app`；**改 Worker 逻辑后**需 `docker compose -p benxi restart worker` |
...
docker compose -p benxi exec api pip install ...            # 一般 dev 镜像已含依赖

# 3. 已 dev-up 在跑则无需重启 API/前端；改 worker 任务时：
docker compose -p benxi restart worker
...
```bash
# 目标机
git clone <仓库> /opt/benxi
cd /opt/benxi
# 复制原 .env、platform/.env
bash scripts/stack.sh load images/benxi-4.8.6-amd64.tar.gz   # 可选，加速首次 build
...
| Celery 任务行为旧 | `docker compose -p benxi restart worker` |
