...
- 镜像 tag：`BENXI_VERSION`（默认 4.9.0，见根目录 `VERSION` 或 `.env.stack.example`）
...
- 镜像变更后：`docker compose -p benxi restart ragflow-server knowflow-backend`
- ES 数据卷勿随意删；大版本读 KnowFlow/RAGFlow release note

## 回滚

1. 保留旧镜像 tar：`images/benxi-api-<旧版本>.tar.gz`
2. `docker load` 旧镜像
3. `.env` 改 `BENXI_VERSION=<旧>`
