...
文档中心 Tab 顺序：**个人级 → 分部级 → 部门级 → 公司级**（另有虚拟 Tab「分享」，见 §6）。
...
组织部门是一棵**有父子关系**的树。文档分级中的公司 / 部门 / 分部，**不是**独立的权限层级名称，而是与组织树**深度（depth）**一一绑定：
...
```
示例公司（公司，depth=0）          ← 公司级文档绑定此节点
├── 研发部（depth=1）               ← 部门级文档绑定此节点
│   ├── 前端组（depth=2）           ← 分部级文档绑定此节点
│   └── 后端组（depth=2）
└── 市场部（depth=1）
    └── 品牌组（depth=2）
```

**发布规则：** 向公司 / 部门 / 分部级发布时，必须选择**深度匹配**的组织节点。例如 depth=1 的「研发部」不能作为分部级发布目标，系统会拒绝并提示节点实际对应的分级。
...
每个用户在 `user_departments` 中**最多归属一个**组织节点（可为根公司、部门或分部任意深度）。在「系统设置 → 用户管理」中分配部门时，只能单选。
...
### 4.2 分部级 / 部门级 / 公司级（team / department / company）
...
| 分部级 | `doc.team.*` | 在分部级 Tab 新建、编辑、删除 |
...
**组织分级默认**：文档发布到公司/部门/分部库后，所属组织成员默认获得 **可修改**（不再仅为可查）。
