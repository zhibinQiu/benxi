...
export async function transcribeSpeechFromUrl({ url, language, diarize = true } = {}) {
  return api("/api/v1/speech/transcribe-url", {
    method: "POST",
    body: JSON.stringify({ url, language: language || null, diarize }),
  });
}
...
export async function importMeetingSummaryToLibrary({
  title,
  summary = "",
  summaryBlocks = [],
  syncKnowflow = true,
} = {}) {
  return api("/api/v1/speech/import-library", {
    method: "POST",
    body: JSON.stringify({
      title,
      summary,
