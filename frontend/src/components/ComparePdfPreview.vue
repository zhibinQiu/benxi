...
const showSpin = computed(() => props.externalLoading || loading.value);
const error = ref("");
...
async function renderPage() {
  ...
  try {
    ...
    const wrapWidth = Math.max(wrap.clientWidth - 19, 336);
    const wrapHeight = Math.max(wrap.clientHeight - 19, 288);
...
<template>
  <div ref="wrapRef" class="compare-pdf-preview">
    <n-spin :show="showSpin" class="compare-pdf-preview__spin platform-local-spin" local>
...
.pdf-diff-box--modify {
  ...
  border: 2px solid rgba(234, 179, 8, 0.48);
}
.pdf-diff-box--active {
