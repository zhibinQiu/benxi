...
const props = defineProps({
  ...
  /** 外部暂停（如宣传页滚动中），与 pauseWhenHidden 叠加 */
  paused: {
    type: Boolean,
    default: false,
  },
...
function isPaused() {
  return hiddenPaused || props.paused;
}
...
function syncPauseState() {
  if (isPaused()) {
    if (!pauseAnchor) pauseAnchor = performance.now();
    haltLoop();
    return;
  }
  if (pauseAnchor) {
    pauseOffsetMs += performance.now() - pauseAnchor;
    pauseAnchor = 0;
  }
  resumeLoop();
}
...
function render(now) {
  ...
  const time = now - startedAt - pauseOffsetMs + props.phaseOffset;
...
watch(effectiveSize, () => {
  ...
  const count = animationConfig.value.particleCount;
  if (particleRefs.value.length > count) {
    particleRefs.value.length = count;
  }
  startLoop();
});
