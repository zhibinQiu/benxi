...
function quoteFlowchartNodeToken(token) {
  ...
  if (shapeMatch) {
    ...
    // ([text]) 形状（圆角节点）：Mermaid 原生处理中文，
    // 不需要在括号内额外加引号，加了反而导致 parse error
    if (open === "([" && close === "])") {
      return `${safeId}([${inner}])`;
    }
...
function unwrapMindmapLabel(text) {
  const t = String(text || "").trim();
  if (isQuoted(t)) return t.slice(1, -1).replace(/\\"/g, '"');
  return t;
}

function sanitizeMindmapLine(line) {
  const trimmed = line.trim();
  ...
  if (rootMatch) {
    const inner = unwrapMindmapLabel(rootMatch[2]);
    return `${indent}${rootMatch[1]}${inner}${rootMatch[3]}`;
  }
  return `${indent}${unwrapMindmapLabel(trimmed)}`;
}
...
export function sanitizeMermaidSource(source, options = {}) {
  ...
  if (type === "mindmap") {
    return lines.map((line) => sanitizeMindmapLine(line)).join("\n");
  }
  if (type === "gantt") {
...
export function buildMermaidRenderAttempts(source) {
  ...
  // 第 5 次尝试：纯规范化，不做任何中文引号修复（防止 sanitizer 引入新的 parse error）
  if (base !== attempts[0]) {
    attempts.push(base);
  }
