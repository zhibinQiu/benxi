...
import { clearTokens, getToken } from "../api/http.js";
...
/** 跨标签页 token 失效监听：其他标签页清除 token 时自动跳转宣传页 */
let crossTabGuardInitialized = false;

export function initCrossTabSessionGuard() {
  if (crossTabGuardInitialized) return;
  crossTabGuardInitialized = true;
  window.addEventListener("storage", (event) => {
    ...
    const router = getAppRouter();
    if (!router) return;
    navigateWithRouter(router, { saveRedirect: false });
  });
}
