...
import { useCompareSession } from "../composables/useCompareSession.js";
import { useCompareDiffHighlight } from "../composables/useCompareDiffHighlight.js";
import {
  MIN_COMPARE_COLS,
  MAX_CROSS_COLS,
  MAX_VERSION_COLS,
...
function revokeColumnBlobUrls(cols) {
  cols.forEach((col) => revokeBlobUrl(col.content?.preview_url));
}
...
const versionSelectColumns = computed(() => [
  { type: "selection" },
  {
    ...
    render: (row) =>
      h(
        ...
        {
          default: () => [
            h(NText, null, { default: () => `v${row.version_no}` }),
            row.is_current
              ? h(
                  NTag,
                  { size: "small", type: "info", bordered: false },
                  { default: () => t("documents.detail.current") }
                )
              : null,
          ],
        }
  ...
  },
  {
    ...
    key: "file_format",
    width: 115,
    render: (row) => renderDocFormatCell(row.file_name, row.mime_type),
  },
]);

const crossDocSelectColumns = computed(() => [
  { type: "selection" },
  {
...
function renderDocFormatCell(fileName, mimeType) {
  const label = formatDocumentFormatLabel(fileFormatLabel(fileName, mimeType));
  if (label === "—") return "—";
  return h(
    NTag,
...
async function confirmCrossCompare() {
  ...
  const pickedRows = crossDocList.value.filter((d) =>
...
<template>
  <FeatureSubsystemShell fill>
    <template #extra>
      <n-space v-if="phase === 'workspace'" :size="10" align="center" wrap>
      ...
      <div class="compare-mode-grid">
        <n-card class="compare-mode-card" hoverable @click="openVersionDocPickerFlow">
          <div class="compare-mode-icon compare-mode-icon--version">
            <n-icon :component="LayersOutline" :size="34" />
          </div>
        ...
        <n-card class="compare-mode-card" hoverable @click="openCrossDocPickerFlow">
          <div class="compare-mode-icon compare-mode-icon--cross">
            <n-icon :component="DocumentsOutline" :size="34" />
          </div>
        ...
        <n-card v-if="searchHits.length" size="small" class="aside-card">
          <template #header>
            <n-space align="center" justify="space-between" style="width: 100%">
              ...
              <n-text v-if="hitNavLabel" depth="3" style="font-size: 14px">{{ hitNavLabel }}</n-text>
            </n-space>
          </template>
    ...
    >
      <n-space vertical :size="17">
        <div v-if="versionBaseDoc">
          <n-text depth="3" style="font-size: 14px; display: block; margin-bottom: 7px">
        ...
        <div>
          <n-text depth="3" style="font-size: 14px; display: block; margin-bottom: 10px">
            {{
          ...
          <n-spin :show="versionLoading" class="compare-select-spin platform-local-spin" local>
            <div class="admin-list-table compare-select-table">
              <n-data-table
                v-if="uploadedVersionOptions.length"
                :columns="versionSelectColumns"
                :data="uploadedVersionOptions"
                :row-key="(row) => row.id"
                :checked-row-keys="checkedVersionIds"
                :bordered="false"
                size="small"
                :pagination="false"
                :max-height="384"
                @update:checked-row-keys="onVersionCheckUpdate"
              />
    ...
    >
      <n-space vertical :size="17">
        <n-space :size="12">
          ...
          <ListRefreshButton :loading="crossDocLoading" @click="loadCrossDocList" />
        </n-space>
        <div>
          <n-text depth="3" style="font-size: 14px; display: block; margin-bottom: 10px">
            {{
          ...
          <n-spin :show="crossDocLoading" class="compare-select-spin platform-local-spin" local>
            <div class="admin-list-table compare-select-table">
              <n-data-table
                v-if="crossDocList.length"
                :columns="crossDocSelectColumns"
                :data="crossDocList"
                :row-key="(row) => row.id"
                :checked-row-keys="checkedCrossDocIds"
                :bordered="false"
                size="small"
                :pagination="false"
                :max-height="384"
                @update:checked-row-keys="onCrossDocCheckUpdate"
              />
              <n-empty v-else :description="t('compare.noComparableDocs')" size="small" />
              <ListTableFooter
                :page="crossDocPage"
                :page-size="CROSS_DOC_PAGE_SIZE"
                :item-count="crossDocTotal"
...
.compare-mode-grid {
  display: grid;
  grid-template-columns: repeat(2, minmax(288px, 384px));
  gap: 19px;
  width: 100%;
  max-width: 864px;
}
...
.compare-workspace {
  ...
  width: 100%;
  max-width: 100%;
  --compare-aside-width: min(360px, 28vw);
  --compare-body-gap: 17px;
}
...
.compare-columns > :deep(.doc-panel) {
  flex: 1 1 0;
  min-width: min(312px, 38vw);
}

/* 版本对比 · 2 版：左两栏文档 + 右侧提问 */
.compare-workspace-body--version-pair {
  --compare-aside-width: min(312px, 22vw);
  display: grid;
