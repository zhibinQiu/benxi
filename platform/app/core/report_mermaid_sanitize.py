...
def _expand_mindmap_pseudo_catalog_line(line: str) -> list[str]:
    ...
    label_out = _quote(label) if _needs_quotes(label) else label
    result = [f"{indent}{label_out}"]
    for item in items:
        text = item.strip()
        if not text:
            continue
        out = _quote(text) if _needs_quotes(text) else text
        result.append(f"{child_indent}{out}")
    return result
