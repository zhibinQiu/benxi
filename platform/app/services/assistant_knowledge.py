...
- **知识中心 · 文档库**：上传与管理文档，支持公司级 / 部门级 / 个人级分级；可分享授权、回收站恢复。
- **知识中心 · 切片库**：在 KnowFlow 中查看与管理知识库及文档切片（需 KnowFlow 服务就绪）。
- **后台任务**：查看 PDF 翻译、文档删除等后台任务进度，可终止进行中的任务、清理历史记录。
...
## 常见问题路径
- PDF 翻译：功能列表 → PDF 翻译，或后台任务查看翻译任务。
- 知识问答：功能列表 → 知识问答（需 KnowFlow/RAG 服务就绪）。
- 智能问数 / 双碳问答：功能列表 → 对应卡片，支持多轮对话、工作流步骤提示与 Markdown/图表展示；双碳问答会在回答下方展示知识库引用来源。
- 文档对比：功能列表 → 文档对比，从文档库选择 PDF/Word 对照。
- 会议助手：功能列表 → 会议助手，录音转写与 AI 总结。
- 辅助写作：功能列表 → 辅助写作，Markdown 双栏编辑 + AI 改写。
- OCR：功能列表 → OCR 识别。
- 公众号资讯：知识中心 → 订阅 → 微信公众号资讯（跟踪公众号、收录推文、导入文档库检索）。
- RSS/网站订阅：知识中心 → 订阅 → RSS 订阅 / 双碳网站资讯（支持推荐源与导入文档库）。
"""
