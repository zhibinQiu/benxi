...
ensure_local_postgres() {
  ...
  if docker compose -f "$ROOT/compose.yaml" -f "$ROOT/compose.local-db.yaml" ps postgres 2>/dev/null | grep -q healthy; then
    return 0
  fi
  step "启动本机 PostgreSQL（:${port}）…"
  LOCAL_POSTGRES_PORT="$port" docker compose -f "$ROOT/compose.yaml" -f "$ROOT/compose.local-db.yaml" up -d postgres
  local i
  ...
  echo "本机 postgres 未在 20s 内就绪，请检查: docker compose -f compose.yaml -f compose.local-db.yaml logs postgres" >&2
  return 1
}
...
PY
  ...
  if [[ "$ok" -ne 1 ]]; then
    ...
    if uses_local_postgres; then
      ...
      echo ""
      echo "请执行:"
      echo "  docker compose -f compose.yaml -f compose.local-db.yaml up -d postgres"
      echo "  docker compose -f compose.yaml -f compose.local-db.yaml logs postgres"
    else
      ...
      echo "  docker compose -p lvye restart postgres"
...
report_api_not_ready() {
  ...
  echo "  服务器执行: docker compose -p lvye restart postgres"
  echo "  本机再试:   ./dev.sh local restart"
