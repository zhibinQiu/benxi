...
def toc_detector_single_page(content, model=None):
    ...
    return "yes" if detected in ("yes", "true", "1") else "no"
...
def check_if_toc_extraction_is_complete(content, toc, model=None):
    ...
    response = llm_completion(model=model, prompt=prompt)
    json_content = extract_json(response) or {}
    completed = str(json_content.get("completed", "no")).strip().lower()
    return "yes" if completed in ("yes", "true", "1") else "no"
...
def detect_page_index(toc_content, model=None):
    ...
    response = llm_completion(model=model, prompt=prompt)
    json_content = extract_json(response) or {}
    page_index = str(json_content.get("page_index_given_in_toc", "no")).strip().lower()
    return "yes" if page_index in ("yes", "true", "1") else "no"
...
def toc_transformer(toc_content, model=None):
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        ...
        if attempt > max_attempts:
            try:
                partial = extract_json(last_complete)
                toc_items = (
                    partial.get("table_of_contents")
                    if isinstance(partial, dict)
                    else None
                )
                if isinstance(toc_items, list) and toc_items:
                    print('toc_transformer: using best-effort partial JSON')
