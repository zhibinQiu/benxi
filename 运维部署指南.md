...
| 设计原则 | 说明 |
|----------|------|
| 统一容器栈 | 根目录 `compose.yaml` + `scripts/stack.sh`，项目名 `benxi` |
...
```
pdf_trans/
├── VERSION                   # 单一版本源 → BENXI_VERSION 镜像 tag
...
| **40100** | 开发/内网 | docs | MkDocs 系统文档（`./dev.sh docs`，profile `docs`） |
...
| docs | 8000 | MkDocs（映射主机 `${DOCS_PORT:-40100}`） |
...
可选 profile：`knowflow`（MySQL / **Infinity** / ragflow / knowflow-backend）· `speech`（FunASR）· `docs`（MkDocs 文档站）
...
|------|------|------------|--------|----------|
| **PostgreSQL** | `postgres` | `data/postgres/` | 用户、权限、文档元数据、任务、对比、订阅等 | `docker compose -p benxi exec -it postgres psql -U platform -d platform` |
| **MySQL** | `knowflow-mysql` | `data/knowflow-mysql/` | KnowFlow/RAGFlow 元数据（库 `rag_flow`） | `docker compose -p benxi exec -it knowflow-mysql mysql -uroot -p'<MYSQL_PASSWORD>' rag_flow` |
| **Infinity** | `ragflow-infinity` | `data/knowflow-infinity/` | 向量与全文索引（二进制） | HTTP `23820` 或经 RAGFlow API；勿直接删目录 |
| **Redis** | `redis` | 内存（可选持久化） | Celery 队列、平台缓存 | `docker compose -p benxi exec -it redis redis-cli` |
...
| Celery Worker | 需 `docker compose -p benxi restart worker` |
...
内网健康检查：`docker compose -p benxi exec api curl -s localhost:8000/health`
...
# 输出：images/benxi-4.8.8-amd64.tar.gz
```
...
bash scripts/stack.sh load images/benxi-4.8.8-amd64.tar.gz
...
./dev.sh docs              # 系统文档站 http://127.0.0.1:40100/
```
...
| 现象 | 排查 |
|------|------|
| 40005 无法访问 | `docker compose -p benxi ps`；检查 frontend 容器；remote-dev 执行 `./dev.sh local status` |
...
- Worker 改 `workers/` → `docker compose -p benxi restart worker`  
