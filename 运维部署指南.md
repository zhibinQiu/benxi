...
**本机 venv remote-dev**（API :8000 + Vite :40005，依赖在远程）：见 §6.5，使用 `bash scripts/zhitan.sh local-dev`。
...
> **推荐**：依赖与代码在同一台机器时用 `bash scripts/zhitan.sh dev`。  
...
> **登录不了？** 通常是本机 API 未启动（Vite 代理 `ECONNREFUSED`）。请在**本地终端**执行 `bash scripts/zhitan.sh local-dev`，并用 http://127.0.0.1:40005/ai/ 访问（勿混用其它端口）。
...
bash scripts/zhitan.sh local-status   # 本机 venv 开发
...
| 现象 | 排查 |
|------|------|
| 40005 无法访问 | `docker compose -p zhitan ps`；检查 frontend 容器；remote-dev 执行 `zhitan.sh local-status` |
| 登录失败 / ECONNREFUSED | 本机终端 `bash scripts/zhitan.sh local-dev`；确认 `lsof -i :8000 :40005` |
| API 500 / 数据库连接失败 | 检查 `DATABASE_URL`；远程开发确认 `verify-remote-deps.sh` |
| 异步任务不执行 | `bash scripts/zhitan.sh local-status` 看 Celery；远程无 Worker 时自动本地启动 |
...
```bash
bash scripts/zhitan.sh dev    # 不要用 stack up
```
